...
async function carregarOrcamentosRecentes() {
    ...
    try {
        ...
        lista.forEach(orc => {
            ...
            // Converte para BRL para exibição unificada
            const moedaOriginal = orc.moeda || 'BRL';
            const cotacao = cotacoes[moedaOriginal] || 1;
            const valorEmBRL = (Number(orc.valor) || 0) * cotacao;

            // Formata: mostra valor na moeda padrão do usuário
            const moedaPadrao = localStorage.getItem('orcafacil_moeda') || 'BRL';
            let displayValor = formatarMoeda(valorEmBRL, moedaPadrao);
...
async function carregarPaginaOrcamentos() {
    ...
    lista.forEach(orc => {
        ...
        const valorEmBRL = (Number(orc.valor) || 0) * cotacao;
        const moedaPadrao = localStorage.getItem('orcafacil_moeda') || 'BRL';
        let displayValor = `<span class="text-green-600">${formatarMoeda(valorEmBRL, moedaPadrao)}</span>`;
...
function formatarMoeda(v, moedaParam = null) {
    const moeda = moedaParam || localStorage.getItem('orcafacil_moeda') || 'BRL';
    const config = {
        'BRL': { locale: 'pt-BR', currency: 'BRL' },
        'USD': { locale: 'en-US', currency: 'USD' },
        'EUR': { locale: 'de-DE', currency: 'EUR' }
    };
    const cfg = config[moeda] || config['BRL'];
...
async function atualizarKPIs() {
    ...
    // Converte todos os valores para BRL antes de somar
    ...
    if (document.getElementById('kpi-faturado')) document.getElementById('kpi-faturado').innerText = formatarMoeda(faturado);
...
async function carregarDetalhes() {
            ...
            <tr class="border-b border-gray-100">
                <td class="p-1 text-left">
                    <div class="flex flex-col">
                        <span class="text-[10px] font-medium text-black">${qtd}x ${item.nome}</span>
                        ${item.descricao ? `<span class="text-[8px] text-gray-500 mt-0.5">${item.descricao}</span>` : ''}
                        ${(subMO > 0 || subFrete > 0) ? `<span class="text-[8px] text-gray-400 mt-0.5">${subMO > 0 ? `M.O: ${formatarMoeda(subMO, orc.moeda)}` : ''} ${subMO > 0 && subFrete > 0 ? ' | ' : ''} ${subFrete > 0 ? `Frete: ${formatarMoeda(subFrete, orc.moeda)}` : ''}</span>` : ''}
                    </div>
                </td>
                <td class="p-1 text-right text-[10px] font-mono text-black font-semibold">${formatarMoeda(valTotalItem, orc.moeda)}</td>
            </tr>`;
        });
    } else {
        // Fallback Legado
    ...
    if (empresa) {
        ...
        // Mostra QR Code se estiver configurado
        if (empresa.pix_qr && qrContainer && qrImg) {
            qrImg.src = empresa.pix_qr;
            qrContainer.classList.remove('hidden');
        }
        
        // Mostra box PIX se tiver QR code OU chave PIX do orçamento OU chave padrão da empresa
        if (boxPix && (empresa.pix_qr || orc.chave_pix || empresa.pix)) {
            boxPix.classList.remove('hidden');
            // Preenche chave PIX (prioridade: orçamento > empresa padrão)
            if (chavePixEl) {
        ...
        if (empresa.cor_primaria) {
            aplicarCoresPDF(empresa.cor_primaria);
        }
    }
}
...
async function salvarConfiguracoesSupabase() {
    ...
    try {
        ...
        let res;
        const payload = { nome, doc, tel, endereco, pix, logo, pix_qr, cor_primaria, cor_secundaria, moeda_padrao };
...
function aplicarCoresPDF(corPrimaria) {
    ...
    if (tableHeader) {
        tableHeader.style.backgroundColor = corPrimaria;
        tableHeader.style.color = isLightColor(corPrimaria) ? '#000000' : '#ffffff';
    }
